...
var FORMAT         = '%H%n%s%n%an%n%b%n' + SEPARATOR;
...
exports.getCommits = function (options) {
  ...
  .then(function (tag) {
    ...
    console.log('git log -E --format=' + FORMAT + ' ' + options.hash)

    return CP.execAsync(
      'git log -E --format=' + FORMAT + ' ' + options.hash, //+ ' ' + revisions,
      {
