...
exports.markdown = function (version, commits, options) {
  var content = [];
  var date = (new Date()).toGMTString() //.toJSON().slice(0, 10);
  var heading;
  ...
  .each(function (type) {
    ...
    Object.keys(this.types[type]).forEach(function (category) {
      ...
      types[type][category].forEach(function (commit) {
        ...
        content.push(prefix + ' ' + subject + ' [' + shorthash + '](https://github.com/FrontlineEducation/msvc-ResourceLibrary/commit/' + shorthash + ')');
  ...
  .then(function () {
    content.push('##### Contributors');
    content.push(Object.keys(contributors).join(', '));
    content.push('');
    content.push('');
    return content.join('\n');
  });
